...
**✅ VERIFICADO EN LA VM (2026-07-20, mismo día) — venta real, app compilada del commit del día, impresora falsa capturando bytes en 127.0.0.1:9100** (mismo mecanismo que la prueba del fix de socket, sin el "lector lento" que ya no hacía falta):
- `cortes: 2` — ticket de venta y comanda llegaron completos, ninguno truncado (confirma el fix de anoche funcionando en la app real, no solo en el test aislado).
- Divisor de marca (`*  *  *  ...`) presente dos veces (tras encabezado y tras TOTAL).
- TOTAL en reverse video confirmado byte a byte: `GS B 1` (`1d 42 01`) inmediatamente antes del texto "TOTAL".
- Logo: 13 apariciones del comando de imagen `ESC *` — coherente con una imagen de ~300px de alto mandada en tiras horizontales, no un comando suelto.
- Comanda con modificadores correctos (`+ Leche de almendra`, `+ Extra shot`).
- **Ojo con `strings` para depurar esto en el futuro:** por default solo entiende ASCII y corta el texto en cualquier acento (é, ó — que la impresora guarda como 1 byte Latin-1) haciendo parecer que el texto está roto cuando NO lo está. Se confirmó con un decodificador Latin-1 real (`d.decode('latin1')` + split solo en bytes de control) que "Café Americano" etc. salen completos y correctos — era un falso positivo de la herramienta, no un bug.
- **Sigue pendiente (no verificable sin la impresora física del sitio):** qué tan oscuro/legible sale el logo impreso de verdad, y si el ancho de 80mm corta algún texto en el papel real.
...
6. ✅ **HECHO Y VERIFICADO DE PUNTA A PUNTA EN LA VM (2026-07-20) — Módulo de actualizaciones.** Ver §7 para el diseño. Se construyó y probó a fondo el **backend** (sin UI todavía — eso queda para hacerlo despierto, viendo la pantalla real):
   ...
   - ✅ **HECHO (2026-07-20, mañana) — pantalla de Actualizaciones**, agregada dentro de `KioskScreen` (`lib/features/admin/screens/kiosk_screen.dart`, la pantalla "Quiosco" ya existente en el shell de admin, junto a Info del equipo/Acciones — se agregó ahí en vez de crear una pantalla nueva). Flujo: botón "Buscar paquete" → `FilePicker.platform.getDirectoryPath()` para elegir la carpeta del USB → lee el manifiesto (`UpdateService.readManifest`) → compara versión (`compareToInstalled`) → muestra versión instalada vs. candidata → botón "Aplicar actualización" (solo habilitado si es más nueva) con diálogo de confirmación → `UpdateService.applyUpdate(packageDir:, installDir: Directory(p.dirname(Platform.resolvedExecutable)))` → si tiene éxito, ofrece "Reiniciar ahora" (`PowerService.restartApp()`, confirmado que SÍ relanza el binario desde su ruta en disco, así que recoge la versión nueva). También se agregó "Versión instalada" al panel de info del equipo.
     - **Sin test automático para esta pantalla** — `KioskScreen._loadExtras()` ya llamaba (antes de esta sesión) a `backupServiceProvider` sin overrides, que golpea el canal de plataforma real de `path_provider`; mockearlo para una sola pantalla no se justificó por tiempo. Verificado con `flutter analyze` limpio + `flutter build windows --debug` exitoso.
   - ✅ **VERIFICADO EN LA VM (2026-07-20, mismo día) — flujo completo real, sin mocks:**
     1. Se simuló una instalación real: bundle compilado copiado a `~/test-opt-latercia/` (haciendo de `/opt/latercia`), app corrida desde ahí (para que `Platform.resolvedExecutable` apunte a esa carpeta, como en producción).
     2. `dart run tool/generate_update_manifest.dart build/linux/x64/release/bundle 1.1.0` — generó el manifiesto (31 archivos) sobre el bundle recién compilado, usado como "paquete nuevo".
     3. En Quiosco → Actualizaciones → "Buscar paquete" → se eligió esa carpeta con el selector nativo de GTK (funcionó sin problema) → la pantalla mostró correctamente **Versión instalada: 1.0.0**, **Versión del paquete: 1.1.0**, **31 archivos**, y el aviso verde "✓ Más nueva que la instalada — se puede aplicar."
     4. "Aplicar actualización" → confirmación (avisando del respaldo automático) → aplicado con éxito → diálogo "Reiniciar ahora".
     5. Confirmado en el sistema de archivos: `test-opt-latercia` (actualizada) + `test-opt-latercia.backup-<timestamp>` (la versión anterior, respaldada automáticamente antes del swap) — exactamente el diseño de `applyUpdate`.
     6. Reinició con "Reiniciar ahora" → la app volvió a abrir normal (pantalla de PIN, sin errores) desde la carpeta ya actualizada — confirma que `PowerService.restartApp()` sí recoge el binario nuevo tras el swap.
     
     **Módulo de actualizaciones: detectar → verificar integridad → aplicar con respaldo → reiniciar — los 4 pasos confirmados con archivos y una app real, no solo con tests aislados.**
   - ✅ **HECHO (2026-07-20) — Manual de instalación corregido y ampliado** (`linux_kiosk/MANUAL_ZERO_TO_PRODUCTION.md`, de 1035 a 1450 líneas). Incorpora todo lo aprendido de la bitácora del 18-jul y de la verificación en VM de hoy:
     - **Fase 4 reescrita**: nueva bifurcación 4.2 (¿un monitor o dos?) — con 2 monitores, el método principal ya no es `cage` (no funciona: junta los monitores en un solo lienzo) sino escritorio normal + autologin + script `wmctrl` (4.3, con el script real `kiosk-launch.sh` de la bitácora, adaptado con placeholders para las coordenadas de cada instalación); `cage` queda como 4.3-alt para el caso de un solo monitor. Incluye el aviso destacado sobre la corrupción de texto de Okular (causa real de casi todos los tropiezos del día de instalación) justo donde más aplica (crear el script a mano).
     - **Correcciones puntuales**: usuario `pos`→variable con aviso explícito; TTY de regreso `Ctrl+Alt+F2` (no F1); rutas de datos actualizadas a las reales post-A3 (`~/.local/share/mx.latercia.pos/` para la base, `~/Documentos/latercia/` para backups/logs) en 3.3b, checklist y Apéndice; CUPS por `lpadmin` (línea de comandos) como vía principal en 5.4, ya que en sitio no había navegador; IP fija por `nmcli` como método principal en 5.3/5.5 (el Tenda del sitio no exponía reserva DHCP con claridad), con aviso del filtro MAC "Lista negra" que casi bloquea la PC/impresora por accidente; diagnóstico `lsof -i :8080` agregado para la botonera; Fase 2.3 instala `wmctrl`/`xdotool` junto con `cage` desde el inicio (no se sabe cuál se va a usar hasta la Fase 4).
     - **Sección nueva — Fase 6.1 "Flujo normal de actualización"**: el proceso completo con el módulo (compilar → subir versión en 2 lugares → generar manifiesto con `tool/generate_update_manifest.dart` → USB → Quiosco → Actualizaciones → Aplicar → Reiniciar), qué pasa detrás (swap atómico + respaldo automático), ramificaciones de error, cómo hacer rollback manual (la UI todavía no tiene botón para esto — limitación conocida, documentada), y aviso de que los respaldos (`/opt/latercia.backup-*`) no se borran solos.
     - **Sección nueva — Fase 6.2 "Transición única"**: el paso especial de la próxima visita, ya que la instalación actual del sitio es de ANTES del módulo de actualizaciones — respaldar la base vieja (`~/Documentos/latercia.sqlite`), reemplazar el bundle a mano, y MOVER la base respaldada a la ruta nueva (`~/.local/share/mx.latercia.pos/`). Marcado explícitamente como algo que se hace una sola vez; de ahí en adelante todo pasa por 6.1.
     - Verificado con una revisión estructural (fences de código balanceados; 8 secciones de nivel 1; sin referencias cruzadas rotas tras la renumeración de Fase 4) — no aplica flutter analyze/test/build por ser documentación.
     - **Pasada extra (mismo día):** a petición explícita del usuario, se convirtieron los 6 bloques `sudo tee archivo <<'EOF'` de la Fase 4 (los `.desktop` de sesión kiosko, `autologin.conf` de SDDM/LightDM) a instrucciones con `nano` en trozos cortos — no solo para el chat en vivo sino para el manual mismo, ya que se va a leer en la máquina del cliente donde puede darse la misma corrupción de copiado que documentó la bitácora del 18-jul. 202 fences de código, balanceados.
...
## 8. Módulo de Backups (✅ HECHO 2026-07-20) + arreglo búsqueda de impresora USB

**Pedido de Jaime:** manejar la base de datos completo desde la app — respaldos, y exportar/elegir qué tablas llevarse, en `.db`/`.sql`/`.xlsx`. Pantalla pensada para él (dueño), no para cajeros.

**Decisión de alcance (acordada antes de programar):** `.db` sigue siendo el ÚNICO formato que la app puede **restaurar** — es una copia completa, atómica, ya probada. `.sql`/`.xlsx` son solo de **exportación** (reportes/revisar datos), con selector de qué tablas incluir (agrupadas: Catálogo, Personas, Operación, Inventario, Envío, Sistema). Reconstruir una BD relacional real desde un `.sql`/Excel editado a mano es donde se corrompen los datos — no vale la pena ese riesgo cuando ya existe un import seguro (`.db`).

**Qué se hizo:**
- `lib/core/services/backup_service.dart`: `exportSql()`/`exportXlsxBytes()` (selección de tablas vía `exportableTables`/`exportGroups`), `listBackups()` (historial completo, antes solo existía el último).
- `lib/features/admin/screens/backups_screen.dart` (nueva pantalla, nuevo destino "Backups" en el menú): automático + retención, respaldar ahora, descargar/restaurar `.db`, historial, selector de tablas con chips y exportar `.sql`/`.xlsx`.
- Se quitó la sección "Respaldo" de Configuración (se movió aquí completa, para no tener el mismo setting editable en dos pantallas).
- Se agregó el paquete `excel` — chocaba de versión con `image` (ambos dependen de `archive` en rangos incompatibles); se resolvió bajando `image` de `^4.5.4` a `^4.3.0` (mismo API que usa el logo del ticket, verificado que sigue compilando y los tests de impresión siguen pasando).
- **Extra pedido en la misma sesión:** al buscar impresora USB en Configuración → Impresión, el resultado (encontrada o no) aparecía tan rápido que se sentía como que no había buscado nada. Se agregó un estado "Buscando impresoras…" visible y se mejoró el mensaje de "no encontrada" (ahora explica que hay que instalarla en Windows primero).
- Tests nuevos: `test/backup_service_test.dart` (+7: listBackups, exportSql, exportXlsxBytes), `test/backups_screen_test.dart` (nuevo, 6 tests), `test/settings_screen_test.dart` (+1, búsqueda de impresora). Suite completa del proyecto: **231/231 verde**, `flutter analyze` limpio.
- **Nota técnica para futuras pantallas con I/O real en `initState`:** en widget tests, si el flujo dispara I/O real de archivos (no solo `Future.delayed`), hay que envolver el propio `pumpWidget`/callback dentro de `tester.runAsync()` — un delay real DESPUÉS de construir la pantalla no alcanza, porque el I/O que arrancó en el zone "fake async" normal de `testWidgets` nunca se completa ahí. Ojo: `tester.tap()` DENTRO de `runAsync()` puede colgar de verdad (visto con un `TimeoutException` a los 10 minutos) — hay que invocar el callback (`onPressed`/`onChanged`) directamente en vez de simular el tap.

### 8.1 Restauración parcial por grupo + asistente de fusión a nivel columna (✅ HECHO 2026-07-20)

**Pedido de Jaime:** poder restaurar SOLO una parte de la base desde un `.db` (solo Clientes, solo Catálogo, solo Inventario…), no todo o nada. Y que el import sea un asistente de fusión: leer el archivo, comparar contra lo actual, y decidir qué se mantiene y qué se trae — al final a nivel **columna**, no solo fila.

**Alcance seguro (grupos que SÍ se pueden restaurar por partes):** Catálogo, Clientes, Empleados, Inventario (datos maestros: proveedores/ingredientes/recetas), Envío. **Excluidos a propósito:** Operación (turnos/órdenes/pagos/gastos/reembolsos/movimientos) y Sistema (settings/audit_log) — historial ligado a todo, o config que pisaría IP de impresora/IVA. Esos solo se restauran con el `.db` completo.

**Cómo funciona (`backup_service.dart`):**
- `previewGroupRestore(backupPath, group)`: abre el `.db` elegido con el paquete `sqlite3` en **solo lectura** (archivo aparte, sin cruzarse con la base viva) y compara fila por fila por `id`. Devuelve `List<RestoreRowDiff>` clasificando cada fila: `nueva` / `igual` / `diferente`, con `currentValues` y `backupValues`.
- `applyGroupRestore(backupPath, group, replace, resolvedRows)`: todo en una transacción drift.
  - **Reemplazar** (`replace: true`): borra el grupo (orden inverso por FK) e inserta lo del respaldo. Si una fila borrada sigue referenciada desde fuera (ej. producto con ventas), la FK lanza y la transacción **revierte completa** — la base queda intacta. Esa es la red de seguridad.
  - **Agregar/fusionar** (`replace: false`): agrega las `nueva`, mantiene las `igual`, y para las `diferente` usa `resolvedRows` (`'tabla:id'` → fila final ya mezclada columna por columna). Un conflicto sin entrada = se mantiene el actual.
- El asistente en la pantalla (`_RestoreWizardDialog`) expande cada fila distinta y, por cada campo que cambió, muestra dos tarjetas Actual/Respaldo para elegir; atajos "Todo actual"/"Todo respaldo" por fila. La UI arma la fila mezclada y la pasa a `resolvedRows`.
- **Pendiente/futuro (no urgente):** comparar campo-por-campo a nivel visual ya está; lo que NO se hizo es un merge automático que "adivine" — siempre lo decide el usuario. OK así.
- Tests: `test/backup_service_test.dart` (+7: preview clasifica, merge mantiene, merge con fila resuelta, **merge a nivel columna mezcla actual+respaldo**, reemplazar deja exacto, reemplazar revierte por FK, grupo no restaurable lanza) y `test/backups_screen_test.dart` (+1: el desplegable ofrece solo grupos seguros, nunca Operación/Sistema). **Suite completa 239/239 verde**, `flutter analyze` limpio.
- **Detalle técnico:** los tests de restauración abren una segunda `AppDatabase` sobre el archivo de respaldo (aislado, se cierra antes de reabrirlo read-only con `sqlite3`). El warning "created AppDatabase multiple times" de drift es benigno aquí (archivos distintos, nunca dos escritores vivos sobre el mismo) y solo sale en debug; se silencia en el test con `driftRuntimeOptions.dontWarnAboutMultipleDatabases = true` (solo limpia la salida, no cambia comportamiento).
